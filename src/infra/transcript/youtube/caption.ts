...
export async function fetchCaptionJson3(
  ...
  if (!resp.ok) throw new SubtitleFetchTransientError(`抓取字幕失败：HTTP ${resp.status}`);
  let json: { events?: Json3Event[] };
  try {
    json = (await resp.json()) as { events?: Json3Event[] };
  } catch {
    throw new SubtitleFetchTransientError("字幕接口返回了无效 JSON");
  }
  ...
  if (out.length === 0) throw new SubtitleUnavailableError("字幕为空");
